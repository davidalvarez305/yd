...
class InvoiceTable(Table):
    receipt = TableField(
        ...
        cell_widget=TableCellWidget(
            data={
                'value': lambda row: (
                    format_html('<a href="{}" target="_blank">Receipt</a>', safe_file_url(row, 'receipt'))
                ) if row.receipt else '',
                'is_html': True
            }
        )
...
class LandingPageTable(Table):
    ...
    revenue = TableField(
        ...
        cell_widget=TableCellWidget(
            data={
                'value': lambda row: (
                    f"${row.visits.aggregate(Sum('lead_marketing__lead__value'))['lead_marketing__lead__value__sum'] or 0:.2f}"
