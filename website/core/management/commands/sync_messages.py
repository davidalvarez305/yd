    ...
    def handle(self, *args, **options):
        ...
        for msg in messages:
            ...
            try:
                ...
                # Only process media if:
                # 1) Message is newly created
                # 2) Or message exists but has no related media
                has_media_in_db = MessageMedia.objects.filter(message=message).exists()
                if not created or (message and not has_media_in_db):
                    for msg_media in msg.get('message_media', []):
                        content_type = msg_media.get('content_type')
                        media_url = msg_media.get('url')
                ...
                self.stderr.write(self.style.ERROR(f"❌ Failed to process message {sid}: {e}"))